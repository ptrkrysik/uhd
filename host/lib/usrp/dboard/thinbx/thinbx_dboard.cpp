    ...
    const double gain, const std::string& name_, const size_t chan)
{
    ...
    // // Default gain profile: Setting anything other than ZBX_GAIN_STAGE_ALL is
    // forbidden if (gain_profile == ZBX_GAIN_PROFILE_DEFAULT && name !=
    // ZBX_GAIN_STAGE_ALL) {
    //     throw uhd::key_error("Invalid gain name for gain profile 'default': " + name);
    // }
    // // Also, when the gain name is all, we have to be in default mode.
...
double thinbx_dboard_impl::set_tx_gain(const double gain, const size_t chan)
{
    ...
    //                                 "set_tx_gain() without a gain name is not
    //                                 allowed!";
...
double thinbx_dboard_impl::set_rx_gain(const double gain, const size_t chan)
{
    ...
    // if (gain_profile == ZBX_GAIN_PROFILE_MANUAL) {
    //     const std::string err_msg = "When using 'manual' gain mode, calling "
    //                                 "set_rx_gain() without a gain name is not
    //                                 allowed!";
    //     RFNOC_LOG_ERROR(err_msg);
    //     throw uhd::runtime_error(err_msg);
    // }
