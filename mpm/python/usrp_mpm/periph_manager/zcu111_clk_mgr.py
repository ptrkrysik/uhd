...
#
"""
ZCU111 Clocking Management

This module handles the analog clocks on the ZCU111 ealuation board.

It is bare minimum version setting up predefined clocks in LMK04208 and
LMX2594 chips.
"""
...
class ZCU111ClockMgr:
    ...
    class PLLSpiOutSelector:
        def __init__(self, log):
            self.gpio0 = Gpio("CLK_SPI_MUX_SEL0", Gpio.OUTPUT, 0)
            self.gpio1 = Gpio("CLK_SPI_MUX_SEL1", Gpio.OUTPUT, 0)
            self.log = log.getChild("LmxLmkOutSel")

        def select_output(self, nr):
            if nr < 0 or nr > 3:
                self.log.error("Incorrect numer of Lmx/Lmk communication output")
                return
            
            self.log.trace(f"Selecting output number {nr}")
            self.gpio0.set(nr & 1)
            self.gpio1.set((nr >> 1) & 1)
    ...
    def __init__(self,
        ...
        # Init peripherals
        self._pll_spi_out_selector = self.PLLSpiOutSelector(self.log)
        self._init_available_srcs()
        self._init_clk_peripherals()
        # Now initialize the clocks themselves
        self._init_ref_clock_and_time( # clock_source,
            # ref_clock_freq,
            sample_clock_freq) # is_legacy_mode, )
    ...
    def _init_clk_peripherals(self):
        ...
        """
        # Create SPI interfaces to the LMK and LMX registers
        dt_name_to_out_nr = {"lmxA": 0, "lmk": 1, "lmxB": 2, "lmxC": 3}

        rpll_spi_node = dt_symbol_get_spidev("lmk")
        reference_pll_regs_iface = lib.spi.make_spidev_regs_iface(
            rpll_spi_node,
            400000,     # Speed (Hz)
            0,          # SPI mode
            0,          # Addr shift
        ...
        self._reference_pll = LMK04208X4xx(
            reference_pll_regs_iface,
            lambda: self._pll_spi_out_selector.select_output(dt_name_to_out_nr["lmk"]),
            self.log
        )

        splls = {}
        for dt_name, out in dt_name_to_out_nr.items():
            if "lmx" in dt_name:
                spll_spi_node = dt_symbol_get_spidev(dt_name)
                spll_regs_iface = lib.spi.make_spidev_regs_iface(
                    spll_spi_node,
                    400000,     # Speed (Hz)
                    0,          # SPI mode (do zapisu działa też wartość 3)
                    16,         # Addr shift
                    0,          # Data shift
                    1<<23,      # Read flag
                    0,          # Write flag
                )
                splls[dt_name] = LMX2594X4xx(
                    spll_regs_iface,
                    lambda: self._pll_spi_out_selector.select_output(out),
                    self.log
                )

        dt_names = list(dt_name_to_out_nr.keys())
        self._adc01_pll = splls[dt_names[0]]
        self._adc23_pll = splls[dt_names[2]]
        self._dac_pll = splls[dt_names[3]]
    ...
    def get_spll_freq(self):
        ...
        return self._adc01_pll.output_freq #TODO PK: zaimplementowac to w obsludze LMXow
    ...
    def _config_rf_plls(self, sample_clock_freq):#, is_legacy_mode):
        """
        Configures the LMXes for the specified master clock rate.
        """
        self._adc01_pll.init()
        self._adc01_pll.config(sample_clock_freq)#, self.get_ref_clock_freq())
        self._adc23_pll.init()
        self._adc23_pll.config(sample_clock_freq)#, self.get_ref_clock_freq())
        self._dac_pll.init()
        self._dac_pll.config(sample_clock_freq)#, self.get_ref_clock_freq())
